    ...
    <div v-if="viewRecord">
      <div style="display: flex; justify-content: space-between;">
        ...
        <div style="display: flex; gap: 10px;">
          <!-- <b-button variant="outline-primary">Edit Medical Record Information</b-button> -->
          <b-button variant="outline-danger" @click="$emit('show-list-of-records')">All Patient Records</b-button>
        </div>
      </div>
      <b-form @submit="onSave" @reset="onReset" v-if="showMedForm">
        ...
        <b-row>
          ...
          <b-col md="6" sm="12">
            <b-form-group label="Select Status" label-for="status">
              <b-form-select v-model="med_form.status" class="mb-3">
                <b-form-select-option :value="null">Please select an option</b-form-select-option>
        ...
        <b-row>
          <b-col md="12" sm="12">
            <b-form-group label="Assesment:" label-for="assessment">
              <b-form-textarea
                id="assessment"
  ...
  props: ['medRecordId'],
    ...
    methods: {
      getAllPatienMedRecords(){
        ...
        axios.get(apiUrl+'get_med_record/'+this.medRecordId).then(response=>{
          console.log(response);
          if (response.data?.result) {
            this.med_form = response.data?.result
          }
        })
      },
      onSave(event){
        event.preventDefault()
        this.med_form.id = this.medRecordId
        axios.post(apiUrl+'update_med_record', this.med_form).then(response=>{
          console.log('response', response);
          if (response.data?.result) {
            this.showMedForm = false
            this.$emit('show-list-of-records', response.data?.result);
          }
        })
      },
      clearData(){
